 ...
 */
const noAsyncEventProperties = {
    meta: {
        type: 'problem',
        docs: {
            description: 'Disallow using specified event properties/methods after an await expression or in promise chains',
            recommended: true,
            url: 'https://github.com/eslint-community/eslint-plugin-async-event',
        },
        messages: {
            noAsyncEventProperties: "Don't access event.{{property}} after an await or in promise chains. Access event properties or call methods before async operations.",
        },
        schema: [
            {
                type: 'object',
            ...
            MemberExpression: (node) => {
                ...
                if (property.type === 'Identifier' &&
                    ...
                    if (isEventObject && objectName) {
                        ...
                        const reported = tracker.isInAsyncContext(objectName, 'noAsyncEventProperties', node, context, { property: property.name });
                        if (reported) {
...
exports.default = noAsyncEventProperties;
